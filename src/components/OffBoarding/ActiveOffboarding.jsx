...
const ActiveOffboarding = ({ data, onNavigate, getStatusColor, getReasonColor }) => {
  return (
    <div className="lg:col-span-2 neu-card p-4 sm:p-6 md:p-8 rounded-3xl">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4 sm:mb-6">
        <div className="mb-2 sm:mb-0">
          <h3 className="text-lg sm:text-xl font-bold text-[#333333] mb-1 sm:mb-2">Active Offboarding</h3>
          <p className="text-sm sm:text-base text-[#666666]">Current employee exit processes</p>
        </div>
        <button
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4">
              <div className="flex items-center space-x-3 sm:space-x-4">
                ...
                <div className="min-w-0">
                  <h4 className="font-bold text-sm sm:text-base text-[#333333] truncate">{employee.name}</h4>
                  <p className="text-xs sm:text-sm text-[#666666] truncate">{employee.designation}</p>
                  <div className="flex flex-wrap items-center gap-x-2 gap-y-1 text-[10px] sm:text-xs text-[#999999] mt-1">
                    <span className="truncate">{employee.department}</span>
                    <span>•</span>
                    <span className="whitespace-nowrap">LWD: {new Date(employee.lastWorkingDay).toLocaleDateString()}</span>
                    <span>•</span>
                    <span className={`px-1.5 py-0.5 rounded-full text-[10px] sm:text-xs ${getReasonColor(employee.reason)}`}>
                      {employee.reason}
                    </span>
                  </div>
                </div>
              ...
              <div className="mt-3 sm:mt-0 text-right">
                ...
                <div className="mt-1.5 sm:mt-2">
                  ...
                  <p className="text-[10px] sm:text-xs text-[#999999] mt-1">Stage: <span className="truncate">{employee.stage}</span></p>
                </div>
              </div>
            </div>
          </div>
...
export default ActiveOffboarding;